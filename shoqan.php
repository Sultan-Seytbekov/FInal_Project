<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SHOQAN</title>
    <link rel="stylesheet" href="Shoqan.css">
    <link rel="icon" href="https://www.picng.com/upload/letter_q/png_letter_q_52255.png">
</head>
<body>
    <div class="main">

        <div class="sticky"> 
            
            <div class="nav">
                <img class="toggle" src="https://cdn-images-1.medium.com/max/280/1*sW40iPA71NdEdbfp2zDVbA@2x.png">
            </div>
            <a href="Shoqan.php"><div class="shoqan" style="font-size: 18px; cursor: pointer; font-weight: 600; font-family: TTHoves, Arial,Helvetica,sans-serif; line-height: 2.3;">SHOQAN</div></a>
            <div class="main_img1">
                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30"><g fill="none" fill-rule="evenodd" stroke="#2D2E2C" stroke-width="1.4" transform="translate(6 8)"><ellipse cx="6.559" cy="6.203" rx="6.413" ry="6.203"></ellipse> <path stroke-linecap="round" d="M13.683 11.754l3.486 2.615"></path></g></svg>
                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30"><g fill="none" fill-rule="nonzero" stroke="#2d2e2c" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.3"><path d="M7 14h17l-2.55 8H9.55zM19 11v-.8C19 8.433 17.433 7 15.5 7S12 8.433 12 10.2v.8"></path></g></svg>
            </div>
        </div>
        
        <a href="2ndpage.php"><div class="back1">
            <button class="back1_btn">NEW COLLECTION</button>
        </div></a>

        <div class="images scrollFade" >
            <div class="c1" >
                <img src="https://shoqansuits.com/static/6e9b1473-e405-40bc-b546-612631056837" alt="">
                <button class="c1_btn">Кэжуал</button>
            </div>
            <div class="c2">
                <img src="https://shoqansuits.com/static/f730fc8e-e497-4518-ac21-dec9e9abc216" alt="">
                <button class="c2_btn">Свадебные</button>
            </div>
            <div class="c3">
                <img src="https://shoqansuits.com/static/a0217f7b-8428-4d47-9b57-02f19e7c47a9" alt="">
                <button class="c3_btn">Классические</button>
            </div>
        </div>

        <div class="tufli scrollFade">
            <a href="third_page.php"><img src="https://shoqansuits.com/static/82e1b845-8c83-46fa-92a6-5e8c358604cd" alt=""></a>
            <a href="third_page.php"><button class="tufli_btn">Посмотреть</button></a>
        </div>

        <h1 class="rek">Рекомендуем</h1>

        <div class="rek_asti scrollFade">
            <div class="bir">
                <img src="https://shoqansuits.com/static/73464d03-495a-49ad-b539-cd75aa988e4c" alt="" width="87%" height="90%">
                <!-- <button class="btn1">Посмотреть</button> -->
            </div>
            <div class="eki">
                <img src="https://shoqansuits.com/static/712388a0-4e09-45d0-a1a4-cbe30588aa51" alt="" width="87%" height="90%">
                <!-- <button class="btn2">Посмотреть</button> -->
            </div> 
        </div>

        <div class="stilist scrollFade">
            <img src="https://shoqansuits.com/static/89b72814-629c-4e44-bd42-4aee49cad1f7" alt="">
            <a href="#foorm"><button class="stilist_btn" >Записаться</button></a>
        </div>

        <h1 class="stilist_text">Бесплатная консультация стилистов</h1>
                                                            
        <div class="mainn scrollFade">
            <div class="aq">
                <img src="https://avatars.mds.yandex.net/get-altay/4797538/2a0000017a0fc3be3ca9903b6097efc67b2a/XXXL" width="550px" height="350px">
            </div>
            <div class="qara">
                <div class="qara_ishi">
                    <h1>Бутики</h1> <br>
                    <p>Найдите ближайший к Вам бутик SHOQAN</p> <br>
                    <p>Посетите бутик SHOQAN в Вашем городе, чтобы <br> окунуться в особенную атмосферу мужского стиля и <br> 
                        получить советы по формированию идеального образа <br> под Ваш запрос.</p>
                    <button class="qara_btn">Магазины</button>
                </div>
            </div>
        </div>

        <h1 class="text scrollFade" id="foorm">Узнай первым</h1>
        <br><br>
        <p class="text_p scrollFade">Подпишись на нашу рассылку и будь первым, кто узнает о предстоящих коллекциях и сотрудничестве. <br>
            Вы также будете получать эксклюзивные предложения, доступные только нашим подписчикам.</p>

            <form class="form scrollFade" action="demo.php" method="post">
                <div class="form__input-group">
                    <p class="email_text">Email</p>
                    <input type="email" class="form__input" name="email">
                </div>
                <div class="form__input-group">
                    <p class="tel">Телефон</p>
                    <input type="text" class="form__input" name="phone">
                </div>
                <div class="form__input-group">
                    <p class="gorod">Город</p>
                    <input type="text" class="form__input" name="gorod">
                </div>
                <button class="form__button" type="submit">Подписаться</button>
            </form>

        <div class="last_text scrollFade">
            <div class="col1">
                <h1>Для клиентов</h1>
                <p>Политика конфиденциальности <br>Условия использования <br> Доставка и возврат <br>Онлайн-платежи</p>
            </div>
            <div class="col2">
                <h1>Контакты</h1>
                <p>WhatsApp: <br><a href="https://api.whatsapp.com/send?phone=+77078443600">+7 707 844 36 00</a> <br> Call-Центр: <br> <a href="">+7 707 844 36 00</a> 
                   <br> По общим вопросам: <br><a href="">shop@shoqansuits.com</a> <br>По поводу заказов: <br><a href="">shop@shoqansuits.com</a></p>
            </div>
            <div class="col3">
                <h1 style="margin-bottom: 40px;">О нас</h1>
                <a href="https://shoqansuits.com/men?gclid=Cj0KCQiAtICdBhCLARIsALUBFcH_uGmpSlbxskU5dgu3toYqoNCr01BKY-ap_f7fBreenChB1EDgx9caAn_VEALw_wcB"><img src="https://i.pinimg.com/736x/9b/e7/a0/9be7a031fcd2215a8827ae719d546182.jpg" width="40px" alt=""></a>
                 <br> <br>
                <a href="https://www.instagram.com/shoqansuits/"><img src="https://www.nicepng.com/png/detail/1-13041_logo-instagram-with-white-circle-background-png-new.png" width="40px" alt=""></a> <br> <br>
                <a href="https://www.pinterest.com/shoqansuits"><img src="data:image/png;base64," width="40px" alt=""></a> 
            </div>
        </div>


     
    

        

    </div>

    <script src="Animation.js"></script>
</body>
</html>